...
2. ✅ SEO优化与预渲染 —— 自动生成sitemap、robots.txt及页面预渲染，极大提升搜索引擎收录与SEO效果
3. ✅ 多邮箱服务支持 —— 可配置多邮箱，提升邮件送达率
4. ✅ 第三方登录集成 —— 支持GitHub、Google、Twitter、Yandex、Gitee等平台登录
5. ✅ 行为验证码功能 —— 集成点选、滑动验证码，防止恶意注册
6. ✅ 看板娘优化 —— Live2D看板娘可自定义、支持AI互动
7. ✅ 导航栏优化 —— 支持自定义导航栏，可以自定义隐藏任何导航栏项，布局更美观
8. ✅ 评论体验优化 —— 评论内容自动保存，未登录也不丢失
9. ✅ 增加兰空图床、简单图床的存储支持 —— 支持多种图片上传和存储方式
10. ✅ AI翻译 —— 支持中英互译，可用本地或API模型
11. ✅ 页脚优化 —— 页脚信息更丰富、可自定义
12. ✅ 图片压缩和转换WebP格式 —— 自动压缩图片，提升网站加载速度
13. ✅ 智能摘要 —— 自动生成文章摘要，提升阅读体验
14. ✅ 暗色模式优化、定时暗色模式 —— 支持夜间自动切换暗色主题，优化暗色模式
15. ✅ 灰色模式 —— 支持全站灰色纪念模式
16. ✅ 自定义错误页面 —— 提供友好的404、403等错误页面
17. ✅ 字体文件CDN化 —— 支持字体文件外部化存储与动态加载，可配置单一/分块字体模式，自定义Unicode范围，大幅减少网站带宽占用
18. ✅ 将MD5密码哈希升级为BCrypt算法 —— 修复密码安全漏洞
19. ✅ 评论区重构、优化其楼层计算算法优化 —— 对评论区进行重构，引入懒加载机制以提升页面加载速度，并使用深度优先遍历算法优化评论楼层计算逻辑，提高渲染性能
20. ✅ Redis缓存优化 —— 大部分接口使用Redis缓存，提升性能
21. ✅ 实现token签名算法HMAC-SHA256认证 —— 完全替换简单UUID token，新增防伪造、防篡改、防重放攻击能力
...
> ⚠️ **注意**：萌趣体是**单字重字体**，不支持粗体变体。这意味着 Markdown 中的 `**粗体**` 语法虽然会被正确解析，但视觉上加粗效果不明显。如需明显的粗体效果，建议更换为支持多字重的字体。
