...
> 
> **注意**：启动后控制台可能会出现关于 `poetize-prerender` 服务的警告日志（如 `I/O error on GET request for "http://poetize-prerender:4000/health"`）。这是因为本地 Java 单独启动时没有启动 Node.js 预渲染服务容器。这是**完全正常**的现象，预渲染服务仅在生产环境（由 Docker Compose 统一启动）提供 SEO 功能，开发环境下该报错不影响任何博客功能的正常使用和调试，可以安全忽略。
