...
```bash
# WebSocket 服务运行在独立端口 9324
# 本地开发：ws://localhost:9324
# 生产环境：通过 Nginx 代理到 wss://你的域名/websocket

# 确保后端服务已启动并监听 9324 端口
docker ps | grep poetize-java
# Linux:
netstat -tlnp | grep 9324
# macOS:
lsof -i :9324
# Windows PowerShell:
netstat -ano | findstr :9324

# 检查 Nginx WebSocket 代理配置
docker exec poetize-nginx cat /etc/nginx/conf.d/default.conf | grep -A 10 "websocket"
```
