...
erDiagram
    ...
    user ||--o{ wei_yan : "发布"
    user ||--o{ resource : "上传"
    user ||--o{ im_chat_user_friend : "好友关系"
    user ||--o{ im_chat_group_user : "群成员"
    
    article ||--o{ comment : "被评论"
    article ||--o{ article_translation : "翻译"
    article }o--|| sort : "属于分类"
    article }o--|| label : "属于标签"
    
    sort ||--o{ label : "包含"
    
    im_chat_group ||--o{ im_chat_group_user : "成员"
    im_chat_group ||--o{ im_chat_user_group_message : "消息"
    
    user ||--o{ im_chat_user_message : "发送私聊"
    user ||--o{ im_chat_last_read : "已读记录"
    
    seo_config ||--o{ seo_search_engine_push : "推送配置"
    seo_config ||--o{ seo_site_verification : "验证配置"
    seo_config ||--|| seo_social_media : "社交配置"
    seo_config ||--|| seo_pwa_config : "PWA配置"
    seo_config ||--|| seo_notification_config : "通知配置"
```
