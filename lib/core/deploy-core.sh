...
create_global_poetize_command() {
  ...
  local poetize_core_dir="$(pwd)/lib/core"
  ...
  sudo mkdir -p "$target_core_dir" 2>/dev/null || return 1
  ...
  if sudo cp "$poetize_script" "$target_path" 2>/dev/null; then
    sudo cp "$poetize_core_dir/"*.sh "$target_core_dir/" 2>/dev/null || return 1
    # 确保复制后的文件具有可执行权限
    sudo chmod +x "$target_path" 2>/dev/null
    sudo chmod +x "$target_core_dir/"*.sh 2>/dev/null
    return 0
  else
    return 1
  fi
...
if [ "${POETIZE_SOURCE_ONLY:-false}" != "true" ]; then
  run_deploy_entrypoint "$@"
fi
