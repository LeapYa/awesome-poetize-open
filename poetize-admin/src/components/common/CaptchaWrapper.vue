<template>
  <div class="captcha-wrapper" v-if="visible">
    ...
    <component
      ...
      v-else-if="configReady && shouldUseCheckboxCaptcha && checkboxCaptchaComponent"
      ref="checkboxCaptcha"
      :action="action"
...
export default {
  ...
  methods: {
    ...
    updateWidth() {
      ...
      const wasLargeScreen = this.configReady ? this.shouldUseCheckboxCaptcha : null;
      
      // 更新宽度
      this.currentWidth = newWidth;
      
      // 如果屏幕类型发生变化（大变小或小变大），重置验证状态
      if (this.configReady && wasLargeScreen !== this.shouldUseCheckboxCaptcha) {
    ...
    createDefaultCaptchaConfig() {
      return {
        ...
        checkbox: { trackSensitivity: 0.90, minTrackPoints: 2 }
      };
    },
    ...
    loadCaptchaConfig() {
      ...
      // 添加加密标识，让后端知道需要加密响应
      this.loadingConfigPromise = this.$http.get(this.$constant.baseURL + "/captcha/getConfig", { encrypted: true }, false)
        .then(res => {
