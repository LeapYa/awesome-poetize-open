 ...
 * 
 * <p>
 * 使用Redis Lua脚本实现原子性限流，支持滑动窗口计数器算法
 * </p>
 * 
 * @author LeapYa
 */
...
public class RateLimitAspect {
     ...
     * </p>
     */
    private static final String RATE_LIMIT_LUA_SCRIPT = "local key = KEYS[1] " +
    ...
    private String buildLimitKey(ProceedingJoinPoint joinPoint, RateLimit rateLimit) {
        // Java 21+ switch 表达式，比旧式 switch 语句更简洁、更安全（没有遗漏 break 的风险）
        return switch (rateLimit.keyType()) {
            case IP -> getClientIp();
            case FINGERPRINT -> getFingerprint();
            case USER -> getUserId();
            case FINGERPRINT_OR_IP -> {
                String fp = getFingerprint();
                yield (fp != null && !fp.isEmpty()) ? "fp:" + fp : "ip:" + getClientIp();
            }
            case CUSTOM -> resolveSpelKey(joinPoint, rateLimit.key());
            default -> getClientIp();
        };
    }
